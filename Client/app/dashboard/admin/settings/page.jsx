...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Toggles */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm max-w-xl space-y-6">
        ...
        {[
          ...
          <div key={toggle.key} className="flex items-start justify-between gap-6 pb-5 border-b border-slate-100 dark:border-slate-800 last:border-b-0 last:pb-0">
            <div className="flex-1">
              <h4 className="text-xs font-bold text-slate-900 dark:text-white">{toggle.title}</h4>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-light mt-0.5 leading-relaxed">{toggle.description}</p>
            </div>
