  ...
  return (
    <div className="space-y-6 p-4 md:p-6 text-slate-800 max-w-[1600px] mx-auto bg-white font-sans min-h-screen">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <div className="text-[10px] uppercase font-mono tracking-wider text-indigo-600 flex items-center gap-1 font-bold"><Users size={12} /> Gross Registrations</div>
          <div className="text-xl font-black text-indigo-600 mt-1">{summaryMetrics.totalEnrollments} Pupils</div>
          <div className="text-[10px] text-slate-500 mt-1 font-mono font-medium">Across active sessions</div>
        </div>
        
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <div className="text-[10px] uppercase font-mono tracking-wider text-slate-500 flex items-center gap-1 font-bold"><TrendingUp size={12} /> Monthly Average</div>
          <div className="text-xl font-black text-slate-900 mt-1">₹{summaryMetrics.averageMonthly.toLocaleString()}</div>
          <div className="text-[10px] text-slate-500 mt-1 font-mono font-medium">Calculated mean yield</div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="text-[10px] text-slate-500 mt-1 font-mono font-medium">Low churn threshold</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-6 border-b border-slate-200 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider font-mono">Revenue Scale Trajectory</h3>
            </div>
            <span className="text-[10px] font-mono text-slate-500 uppercase font-bold">Amounts in INR (₹)</span>
          </div>

          {/* Real CSS Analytics Bars Container */}
          <div className="flex items-end justify-between h-48 pt-4 px-2 font-mono text-[10px] text-slate-500 border-b border-slate-200">
            {monthlyRevenue.map((item, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-center gap-2 w-1/6 group h-full justify-end">
                  ...
                  <div className="opacity-0 group-hover:opacity-100 bg-slate-900 text-white rounded px-1.5 py-0.5 text-[9px] mb-1 transition-opacity shadow-md font-bold">
                    ₹{item.revenue / 1000}k
                  </div>
                  <div style={{ height: barHeight }} className="w-8 sm:w-10 bg-slate-100 group-hover:bg-indigo-600 rounded-t-lg transition-all duration-300 border border-slate-200 group-hover:border-transparent" />
                  <span className="mt-1 font-sans font-bold text-slate-500 group-hover:text-slate-900 transition-colors">{item.month}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <div className="flex items-center gap-2 mb-4 border-b border-slate-200 pb-3">
            ...
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider font-mono">Curriculum Scale Distribution</h3>
          </div>
            ...
            {levelDistribution.map((dist, idx) => (
              <div key={idx} className="text-xs space-y-1">
                <div className="flex justify-between items-center font-mono">
                  <span className="font-bold text-slate-700">{dist.level}</span>
                  <span className="text-slate-500 font-medium">{dist.students} Pupils <span className="text-slate-900 font-black">({dist.percentage}%)</span></span>
                </div>
                {/* Horizontal Scale Tracker */}
                <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                  <div style={{ width: `${dist.percentage}%` }} className={`h-full ${dist.color === 'bg-[#4a5d4e]' ? 'bg-indigo-600' : dist.color} rounded-full`} />
                </div>
              </div>
            ))}
          </div>

          <div className="pt-3 border-t border-slate-200 mt-4 text-[10px] text-slate-500 font-mono font-medium leading-relaxed">
