...
export default function TeacherLayout({ children }) {
  ...
  const teacherName = isClientMounted && user?.name ? user.name : "Teacher Admin";
  const teacherEmail = isClientMounted && user?.email ? user.email : "";
  const teacherPhoto = isClientMounted && user?.profilePhoto ? user.profilePhoto : null;
  const teacherInitials = isClientMounted && teacherName
    ? teacherName.split(" ").filter(Boolean).map(n => n[0]).join("").toUpperCase().slice(0, 2)
    : "TA";
    ...
    <div className="flex h-screen bg-[#FFF8F0] dark:bg-[#150e2a] text-[#1a1035] dark:text-[#f0ebff] font-sans antialiased transition-colors duration-300">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white dark:bg-[#150e2a] border-r border-[#3d2a88]/15 dark:border-[#3d2a88]/30 p-6 justify-between shrink-0 shadow-sm">
        ...
        <div className="border-t border-[#3d2a88]/15 dark:border-[#3d2a88]/30 pt-4">
          ...
          >
            {teacherPhoto || user?.profilePhoto ? (
              <img
                src={teacherPhoto || user?.profilePhoto}
                alt={teacherName}
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate group-hover:text-[#FF6B2B] transition-colors">{teacherName}</p>
        ...
        <header className="h-16 shrink-0 bg-white/90 dark:bg-[#0f0a1e]/90 border-b border-slate-200 dark:border-[#3d2a88]/30 px-4 md:px-8 flex items-center justify-between shadow-sm relative z-30 text-slate-800 dark:text-white backdrop-blur-md">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <div className="relative">
              ...
              {/* Teacher Profile Header Dropdown Menu */}
              {userProfileOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-60 bg-white dark:bg-[#1a1035] border border-slate-200 dark:border-[#3d2a88]/40 rounded-2xl p-3 shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="flex items-center gap-3 p-2 pb-3 border-b border-slate-100 dark:border-[#3d2a88]/30">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">{teacherName}</h4>
                        {teacherEmail && <p className="text-[10px] text-slate-400 truncate">{teacherEmail}</p>}
            ...
            >
              ...
              <div className="w-32 h-32 mx-auto rounded-full bg-gradient-to-br from-[#2D1B69] to-[#FF6B2B] p-1 shadow-xl relative group">
                <div className="w-full h-full rounded-full overflow-hidden bg-white dark:bg-[#150e2a] flex items-center justify-center">
                  {teacherPhoto || user?.profilePhoto ? (
                    <img src={teacherPhoto || user?.profilePhoto} alt={teacherName} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-3xl font-black text-[#FF6B2B]">{teacherInitials}</span>
                  )}
                </div>
              </div>

              <div>
                <h3 className="text-lg font-black text-slate-900 dark:text-white">{teacherName}</h3>
                {teacherEmail && <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{teacherEmail}</p>}
                <div className="inline-flex items-center gap-1.5 mt-2 px-3 py-1 rounded-full text-xs font-bold bg-orange-100 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-900/50">
                  <span>Teacher / Faculty Head</span>
                </div>
              </div>
